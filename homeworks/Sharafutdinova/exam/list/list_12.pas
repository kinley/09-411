...
begin
  ...
  begin
    ...
    if l1.current^.data < l2.current^.data then begin
      ...
      in_end(l3, l2.current^.data); 
      next(l2);
    end;
  end;
  //если один из списков закончился, оставшиеся элементы второго записываем в конец
  if l2.current^.next <> nil then while l2.current^.next <> nil do 
    begin
      in_end(l3, l2.current^.data);
      next(l2);
    end;
